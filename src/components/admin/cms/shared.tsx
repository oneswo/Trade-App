...
export function TextInput({ name, defaultValue = "" }: { name: string; defaultValue?: string }) {
...
export function TextArea({ name, defaultValue = "", rows = 3 }: { name: string; defaultValue?: string; rows?: number }) {
  const { get, set } = useContext(CmsCtx);
  return (
    <textarea
      value={get(name, defaultValue)}
...
export function DarkInput({ name, defaultValue = "", gold }: { name: string; defaultValue?: string; gold?: boolean }) {
  const { get, set } = useContext(CmsCtx);
  return (
    <input
      type="text"
...
export function DarkArea({ name, defaultValue = "", rows = 2 }: { name: string; defaultValue?: string; rows?: number }) {
  const { get, set } = useContext(CmsCtx);
  return (
    <textarea
      value={get(name, defaultValue)}
...
export function CardBlock({ index, label, children }: { index: number; label: string; children: React.ReactNode }) {
  return (
